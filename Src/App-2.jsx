...
export default function App() {
  ...
  const [paid, setPaid] = useState(() => {
    try {
      const s = localStorage.getItem("br2_paid");
  ...
  useEffect(() => {
    try { localStorage.setItem("br2_paid", JSON.stringify(paid)); } catch {}
  }, [paid]);
  ...
  const latestWeekMemberFines = MEMBERS.map(m => {
    const missed = allDates.filter(d => d !== today && !readers(d).includes(m.id)).length;
    return missed * FINE;
  });
  const latestWeekTotalFine = latestWeekMemberFines.reduce((a, b) => a + b, 0);
      ...
      {tab === "check" && (
        <div style={{ padding: "14px 14px" }}>
          ...
          <div style={{ marginTop: "16px" }}>
            ...
            <div style={{ background: C.card, borderRadius: "14px", border: `1px solid ${C.border}`, overflow: "hidden" }}>
              {MEMBERS.filter(m => allMemberFines[m.id] > 0 || paid[m.id]).map((m, i, arr) => (
                <div key={m.id} style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "12px 16px",
                  borderBottom: i < arr.length - 1 ? `1px solid ${C.border}` : "none",
                  background: paid[m.id] ? "#f0fbf4" : C.card,
                }}>
                  <div>
                    <span style={{ fontSize: "14px", fontWeight: "700" }}>{m.id}</span>
                    <span style={{ fontSize: "11px", color: C.sub, marginLeft: "5px" }}>{m.title}</span>
                    <div style={{ fontSize: "12px", fontWeight: "700", color: paid[m.id] ? C.green : C.red, marginTop: "2px" }}>
                      {paid[m.id] ? "입금완료" : `${allMemberFines[m.id].toLocaleString()}원 미납`}
                    </div>
                  </div>
                  <button onClick={() => togglePaid(m.id)} style={{
                    ...
                    background: paid[m.id] ? "#f2f2f7" : C.green,
                    color: paid[m.id] ? C.sub : "#fff",
                  }}>
                    {paid[m.id] ? "취소" : "입금완료"}
                  </button>
                </div>
              ))}
              {MEMBERS.filter(m => allMemberFines[m.id] > 0 || paid[m.id]).length === 0 && (
          ...
          <div style={{ marginBottom: "12px" }}>
            <div style={{
              ...
              <div style={{ fontSize: "11px", color: C.sub, marginTop: "4px" }}>
                4/30 ~ {(() => { const dt = new Date(today); return `${dt.getMonth()+1}/${dt.getDate()}`; })()} 기준
              </div>
            </div>
          </div>
              ...
              return (
                <div key={m.id} style={{
                  ...
                  <div style={{
                    ...
                    color: (allMemberFines[m.id] === 0 || paid[m.id]) ? C.green : C.red,
                  }}>
                    {allMemberFines[m.id] === 0 || paid[m.id] ? "없음" : `${allMemberFines[m.id].toLocaleString()}원`}
                  </div>
                </div>
              );
            })}

...
export default function App() {
  const [records, setRecords] = useState(() => {
    try {
      ...
      // 기존 데이터가 배열 형태면 새 형식으로 자동 변환
      const migrated = {};
      Object.entries(parsed).forEach(([date, val]) => {
        if (Array.isArray(val)) {
          migrated[date] = { normal: val, diamond: [] };
        } else {
          migrated[date] = val;
        }
      });
  ...
  const remainFine = (memberId) => {
    ...
    return fine - bonus - paid; // 마이너스 가능 (상금이 기부금 초과 시)
  };
        ...
        <div style={{ padding: "14px" }}>
          ...
          <div style={{ display: "flex", gap: "10px", marginBottom: "12px" }}>
            <div style={{
              ...
              <div style={{ fontSize: "11px", color: C.sub, marginBottom: "5px" }}>기부금 합계</div>
              <div style={{ fontSize: "22px", fontWeight: "800", color: C.red }}>
                {MEMBERS.reduce((acc, m) => acc + Math.max(0, remainFine(m.id)), 0).toLocaleString()}원
              </div>
            </div>
            <div style={{
              flex: 1, background: C.card, borderRadius: "12px", padding: "14px",
              ...
              <div style={{ fontSize: "11px", color: C.sub, marginBottom: "5px" }}>💎 상금 합계</div>
              <div style={{ fontSize: "22px", fontWeight: "800", color: "#f6c90e" }}>
                {MEMBERS.reduce((acc, m) => acc + (allMemberBonus[m.id] || 0), 0).toLocaleString()}원
              </div>
            </div>
          </div>
              ...
              return (
                <div key={m.id} style={{
                  ...
                  <div style={{ paddingRight: "8px", textAlign: "right" }}>
                    {(() => {
                      const fine = allMemberFines[m.id] || 0;
                      const bonus = allMemberBonus[m.id] || 0;
                      const paid = paidAmount[m.id] || 0;
                      const result = fine - bonus - paid;
                      const color = result < 0 ? "#007aff" : result === 0 ? C.green : C.red;
                      const label = result === 0 ? "없음" : result < 0 ? `-${Math.abs(result).toLocaleString()}원` : `${result.toLocaleString()}원`;
                      return <div style={{ fontSize: "12px", fontWeight: "800", color }}>{label}</div>;
                    })()}
                  </div>
                </div>
              );
